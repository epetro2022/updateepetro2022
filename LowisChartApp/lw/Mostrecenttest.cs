...
{
    public partial class Mostrecenttest : DevExpress.XtraEditors.XtraUserControl
    {
        DB dbCon;
        public Mostrecenttest()
        {
            InitializeComponent();
            dbCon = new DB(Globals.getstringco());
        }

        private void Mostrecenttest_Load(object sender, EventArgs e)
        {
            dbCon.clearCache();
            Dictionary<string, object> paras = new Dictionary<string, object>();
            paras.Add("@param", "");
            DataSet ds = dbCon.execprosedure("dbo.[prosedur_getdataMostrecenttest]", paras);
